...
A one-page application site, Bar Moshe, presented in OurCrowd's own brand, sent
...
All brand values live in `src/marketing/ourcrowd/ourcrowd.css` (`.ourcrowd-root` tokens).
...
```bash
npx vercel --prod          # → https://bar-for-ourcrowd.vercel.app
```
