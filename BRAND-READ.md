# BRAND-READ: replicate the company, don't recolour the ourcrowd

The ourcrowd gives you the plumbing (config, a11y, reveal, CTAs, section spine).
...
motion feel, not the ourcrowd with new colours. Work top to bottom.
...
- **Colours**: `getComputedStyle` on `body`, headings, buttons, links for exact
  hex. Fill `--ourcrowd-bg / --ourcrowd-surface / --ourcrowd-border / --ourcrowd-ink / --ourcrowd-ink-soft /
  --ourcrowd-accent / --ourcrowd-accent-ink` in `src/marketing/ourcrowd/ourcrowd.css`.
- **Type**: the real display + body faces. Their brand face is often licensed;
  ...
  Lato). Expose it as `--ourcrowd-font`.
- **Shape**: corner radius (`--ourcrowd-radius`; `0` for square brands like WeDev),
...
spine in `OurcrowdApp.tsx` to mirror how *they* structure a page.
