# CLAUDE.md: bar-for-ourcrowd (application site)
...
**Replicate the company, do not recolour the ourcrowd.** Everything a visitor
...
live off their site. The ourcrowd only gives you the plumbing. See `BRAND-READ.md`
...
- Next.js 16 (App Router, Turbopack) + React 19 + TypeScript. Plain CSS, **no
  Tailwind**. Deployed on Vercel at `https://bar-for-ourcrowd.vercel.app`.
- Brand tokens live at the top of `src/marketing/ourcrowd/ourcrowd.css`, scoped to
  `.ourcrowd-root`. Re-brand from there.
- `src/marketing/ourcrowd/marketing-base.css` is the brand-agnostic reset + a11y
...
- The GH-Pages export fallback (ADR 0169) is wired in `next.config.ts`; the CV
  link is base-path-safe in `OurcrowdApp.tsx`.
